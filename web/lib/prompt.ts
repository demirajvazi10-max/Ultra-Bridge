...
export function buildSystemPrompt(parts: {groqContext: string; kbOutline?: string}): string {
  const kb = parts.kbOutline
    ? `\n\nKnowledge base: only for open-ended guidance questions such as "what should I learn first", "why does X exist" or "how are the two screen readers different overall". Use knowledge_base_read with a path from this outline:\n${parts.kbOutline}\nNever use knowledge_base_read to answer a question that names a specific command, key or action, even if a guide mentions it in passing: always query the command data with groq_query for those, so you get the command's own verification level and sources, not the guide's.`
    : ''
...
1. Never state a key from memory. Query first, answer from the result. If the data has no answer, say so plainly. A question that names a specific command, key or action is answered with groq_query on the command data, never with knowledge_base_read alone, so the verification level and sources are the command's own.
...
3. Write NVDA commands exactly as the data does, in the form NVDA+X, and say once that the NVDA key can be Insert or CapsLock. Insert+X asked about in NVDA means NVDA+X: look up NVDA+X. A command with layout both is the same in either layout. If the user gives no layout, give desktop and laptop when both exist.
...
6. When you named a command, end with a line starting with Sources: and list only the sources that appear in the query result, with their exact titles and URLs. Never write a URL that is not in the result. If the answer comes from a guide, write: Source: Ultra Bridge guide, followed by its title. If no source is recorded, say so.
